...
number_discrete_values_in_features = np.array([2]*n_features)
# number_discrete_values_in_features = np.array([2]*n_features)
# number_discrete_values_in_features = np.array([2]*n_features)
...
kmeans = Kmeans(init="k-means++", n_clusters=n_clusters, n_init= 10, max_iter = 30, algorithm = "probabilistic_rounding_large_alphabets", t = 2)
# kmeans = Kmeans(init="k-means++", n_clusters=n_clusters, n_init= 20, max_iter = 20, algorithm = "majority_rounding_large_alphabets")


# kmeans = Kmeans(init="seeding2", n_clusters=n_clusters, n_init=1)
kmeans.fit(data, number_discrete_values_in_features = number_discrete_values_in_features, true_labels = labels) #, input_seed = seed)
