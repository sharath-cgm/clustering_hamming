...
def probabilistic_rounding(X, centers_init, max_iter, tanh_t, t, true_labels = None):
	...
	for iter in range(max_iter):
		...
		rounded_fraction = fraction_of_ones**t
		rounded_fraction /= rounded_fraction.sum()
		...
		# for j in range(n_samples):
		# 	kmode_score_current += pairwise_distance[j][labels[j]]

		# compute kmode-score

		# if kmode_score_previous != -1 and kmode_score_current > kmode_score_previous:
		# 	# print(chances)
		# 	print("current: ", kmode_score_current, "previous: ", kmode_score_previous)
		# 	# if chances == 0:
		# 	# 	break
		# 	# chances -= 1
		# 	continue
		...
		# kmode_score_previous = kmode_score_current
	...
	# Majority step as the final labelling
	# pairwise_distance = euclidean_distances(X = X, Y = center_old, squared = True) # Squared euclidean gives similar comparison as hamming distance
	...
	# print("K-mode score over the iterations: ", kmode_score)
	print(" ")
...
# def lloyds(X, centers_init, x_squared_norms, max_iter = 300, tol=1e-4):
def majority_rounding(X, centers_init, max_iter, true_labels = None):
	n_samples, n_features = X.shape
	n_clusters = centers_init.shape[0]
